...
def lemmatization(msg):
    ...
    doc = nlp(msg)
    tokenlist = [
        token.lemma_ for token in doc if str(token) not in set(nlp.Defaults.stop_words)
    ]
    msg = " ".join(tokenlist)
    return msg
...
if stopwords_cond:
    ...
    stopwords.words("english").extend(STOPWORDS)
...
count_lbl = (
    pd.DataFrame(labels_dataframe.sum(axis=1).sort_values(ascending=False))
    .reset_index()
    .groupby(0)
    .count()
    .reset_index()
...
plt.subplots(figsize=(10, 9))
ax = sns.barplot(x=count_lbl["num_lbl"], y=count_lbl["count"], color="#833788")
...
plt.xlabel("Tamanho da mensagem (quantidade de caracteres)", fontsize=13)
plt.title(
    "Histograma do tamanho das mensagens com tamanho menor ou igual a 500", fontsize=16
)
...
wordlist = [
    word for word in my_msg_clean.split() if word not in set(stopwords.words("english"))
]
